...
export async function resolveShader(opts) {
    ...
    for (const reflectedEntry of reflection.entryPoints) {
        ...
        if (emittedEntry?.bindings)
            Object.defineProperty(reflectedEntry, "bindings", { value: emittedEntry.bindings, enumerable: false });
        if (emittedEntry?.samplingPairs)
            Object.defineProperty(reflectedEntry, "samplingPairs", { value: emittedEntry.samplingPairs, enumerable: false });
    }
    const map = sourceMap(modules);
    const minify = normalizeMinifyOption(opts.minify);
    if (opts.validate !== false)
        await validateWGSL(emittedWgsl);
    const wgsl = applyMinifyWgsl(emittedWgsl, minify);
    if (opts.validate !== false && minify.identifiers === "safe")
...
async function loadGraph(path, opts, loaded, stack, diagnostics) {
    ...
    for (const imp of module.parsed.imports)
        await loadGraph(resolvePath(imp.from, path, opts, diagnostics), opts, loaded, stack, diagnostics);
    stack.pop();
}
